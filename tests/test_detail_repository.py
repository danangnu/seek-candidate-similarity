...
class DetailRepositoryTest(unittest.TestCase):
    def setUp(self):
        ...
        CREATE TABLE seek_scrap_detail (id_detail INTEGER PRIMARY KEY, seekid_detail INTEGER UNIQUE, seek_scrap_id TEXT, match_path_found TEXT);
    ...
    def test_pending_and_write_use_detail_columns_only(self):
        ...
        self.assertEqual(self.db.execute('SELECT seek_scrap_id FROM seek_scrap_detail').fetchone()[0],UID)
    ...
    def test_detail_rollback_restores_only_detail_uuid(self):
        ...
        self.assertIsNone(self.db.execute('SELECT seek_scrap_id FROM seek_scrap_detail').fetchone()[0])
        self.assertEqual(self.db.execute('SELECT uuid FROM seek_scrap').fetchone()[0],'existing-main-value')
        self.assertEqual(self.repo.rollback(change),0)
    def test_nonblank_old_integer_link_not_overwritten(self):
        self.db.execute("UPDATE seek_scrap_detail SET seek_scrap_id='12345'")
    ...
    def test_other_numeric_id_uuid_conflict(self):
        self.db.execute('INSERT INTO seek_scrap_detail (id_detail,seekid_detail,seek_scrap_id) VALUES (101,43,?)',(UID,))
        with self.assertRaises(ValueError):self.apply()
        self.assertIsNone(self.db.execute('SELECT seek_scrap_id FROM seek_scrap_detail WHERE seekid_detail=42').fetchone()[0])
    ...
    def test_intervening_change_blocks_rollback(self):
        change,_=self.apply()
        self.db.execute("UPDATE seek_scrap_detail SET seek_scrap_id='changed-after-save'")
        with self.assertRaises(ValueError):self.repo.rollback(change)
        self.assertEqual(self.db.execute('SELECT seek_scrap_id FROM seek_scrap_detail').fetchone()[0],'changed-after-save')
